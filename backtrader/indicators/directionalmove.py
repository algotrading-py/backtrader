...
class DirectionalIndicator(_DirectionalIndicator):
    """
    Определено Дж. Уэллсом Уайлдером-младшим в 1978 году в его книге *"Новые концепции в
    технических торговых системах"*.

    Предназначен для измерения силы тренда

    Этот индикатор показывает +DI, -DI:
      - Используйте PlusDirectionalIndicator (PlusDI), чтобы получить +DI
      - Используйте MinusDirectionalIndicator (MinusDI), чтобы получить -DI
      - Используйте AverageDirectionalIndex (ADX), чтобы получить ADX
      - Используйте AverageDirectionalIndexRating (ADXR), чтобы получить ADX, ADXR
      - Используйте DirectionalMovementIndex (DMI), чтобы получить ADX, +DI, -DI
      - Используйте DirectionalMovement (DM), чтобы получить ADX, ADXR, +DI, -DI
    ...
    Используемое скользящее среднее - это то, которое изначально определил Уайлдер,
    сглаженное скользящее среднее (SmoothedMovingAverage)
