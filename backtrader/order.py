...
class OrderExecutionBit(object):
    """
    Предназначен для хранения информации о выполнении заявки. "Бит" не определяет,
    была ли заявка полностью/частично исполнена, он просто хранит информацию.

    Атрибуты:

      - dt: время исполнения в формате datetime (float)
      - size: сколько было исполнено
      - price: цена исполнения
      - closed: сколько из исполненного объема закрыло существующую позицию
      - opened: сколько из исполненного объема открыло новую позицию
      - openedvalue: рыночная стоимость "открытой" части
      - closedvalue: рыночная стоимость "закрытой" части
      - closedcomm: комиссия за "закрытую" часть
      - openedcomm: комиссия за "открытую" часть

      - value: рыночная стоимость для всего объема бита
      - comm: комиссия за все исполнение бита
      - pnl: прибыль/убыток, сгенерированный этим битом (если что-то было закрыто)

      - psize: текущий размер открытой позиции
      - pprice: текущая цена открытой позиции
      ---------------------------------------------------------
...
class OrderData(object):
    """
    Хранит фактические данные заявки для Создания и Исполнения.

    В случае Создания - это сделанный запрос, а в случае Исполнения -
    фактический результат.

    Атрибуты:

      - exbits: итерируемый объект из OrderExecutionBits для этого OrderData

      - dt: время создания/исполнения в формате datetime (float)
      - size: запрошенный/исполненный объем
      - price: цена исполнения
        Примечание: если цена не указана и не задан лимит цены, в качестве
        ориентира будет использована цена закрытия на момент создания заявки
      - pricelimit: содержит ценовой лимит для StopLimit (который срабатывает первым)
      - trailamount: абсолютное ценовое расстояние в трейлинг-стопах
      - trailpercent: процентное ценовое расстояние в трейлинг-стопах

      - value: рыночная стоимость для всего объема
      - comm: комиссия за все исполнение
      - pnl: прибыль/убыток, сгенерированный этим исполнением (если что-то было закрыто)
      - margin: маржа, затраченная на заявку (если есть)
    ...
    # Согласно документации, collections.deque является потокобезопасным при добавлении
    # с обоих концов, операций pop не будет (нигде), и поэтому для определения новых
    # exbits нужны два индекса. Во время клонирования (__copy__) индексы могут быть
    # обновлены, чтобы соответствовать предыдущему концу и новому концу (len(exbits))
    # Пример: старт 0, 0 -> islice(exbits, 0, 0) -> []
    # Один добавлен -> копия -> обновлено 0, 1 -> islice(exbits, 0, 1) -> [1 элемент]
    # Другой добавлен -> копия -> обновлено 1, 2 -> islice(exbits, 1, 2) -> [1 элемент]
    # "add" и "__copy__" всегда происходят в одном и том же потоке (во всех текущих
    # реализациях), и поэтому во время копирования не произойдет добавления, и
    # длину exbits можно запрашивать без опасений, что другой поток выполнит
    # добавление, и без необходимости в блокировке
    # --------------------------------------------------------------------------
