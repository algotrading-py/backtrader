...
def num2date(x, tz=None, naive=True):
    # То же самое, что и в matplotlib, за исключением случая, когда tz равен None,
    # тогда будет возвращен наивный объект datetime (без информации о временной зоне).
    """
    *x* - это значение с плавающей точкой, которое указывает количество дней
    (дробная часть представляет часы, минуты, секунды) с начала
    0001-01-01 00:00:00 UTC *плюс* *один*.
    Добавление единицы здесь является историческим артефактом. Также обратите внимание,
    что предполагается использование Григорианского календаря; это не является
    универсальной практикой. Подробности смотрите в документации модуля.

    Возвращаемое значение - экземпляр класса :class:`datetime` в часовом поясе *tz*
    (по умолчанию используется значение TZ из rcparams).

    Если *x* является последовательностью, будет возвращена последовательность
    объектов :class:`datetime`.
    """
